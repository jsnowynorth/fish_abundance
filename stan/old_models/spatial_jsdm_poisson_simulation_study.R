...
lambda = dat$effort * exp(matrix(rep(beta_0, N), N, byrow = T) + dat$X %*% beta + each_lake %*% omega + dat$Z %*% phi)

set.seed(1)
Y = apply(lambda, c(1,2), rpois, n=1)
dat$Y = Y

dat$Y = dat$Y[,2:3]
dat$effort = dat$effort[,2:3]
dat$y_vec = c(dat$Y)
dat$K = 2
dat$fish_names = dat$fish_names[2:3]
...
saveRDS(out, '/Users/joshuanorth/Desktop/stan_lake_random_effect_simulation.rds')
