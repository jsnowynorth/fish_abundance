...
gen_Y = function(beta_0, beta, phi, Sigma_species, tau, omega, dat){
  ...
  # z = matrix(rnorm(length(omega)), dim(omega))
  # omega = z %*% chol(Sig_tmp)
  # OMEGA = dat$each_lake %*% (omega - mean(omega))
...
# A = rep(1, length(omega))
# Q = kronecker(solve(Sig_tmp), diag(rep(1, dim(omega)[1])))

# t(A) %*% Q %*% A

# sum(solve(Sig_tmp))*dim(omega)[1]
# sum(Q)

# unique(Q %*% A)

# solve(Sig_tmp) %*% rep(1, 6)

# apply(solve(Sig_tmp), 2, sum)

# rep matrix n_lake times
# unique(Q %*% A) %*% (1/(t(A) %*% Q %*% A)) %*% sum(z)

omega = z - matrix(rep(t(solve(Sig_tmp) %*% rep(1, dat$K) %*% (1/(sum(solve(Sig_tmp))*dat$n_lakes)) %*% sum(z)), dat$n_lakes), ncol = dat$K, byrow = T)

# matrix(Q %*% A %*% (1/(t(A) %*% Q %*% A)) %*% sum(z), 1003, 6)
# z - matrix(Q %*% A %*% (1/(t(A) %*% Q %*% A)) %*% sum(z), 1003, 6)
...
omega_train = z_train - matrix(rep(t(solve(Sig_tmp) %*% rep(1, dat_train$K) %*% (1/(sum(solve(Sig_tmp))*dat_train$n_lakes)) %*% sum(z_train)), dat_train$n_lakes), ncol = dat$K, byrow = T)
...
omega_test = z_test - matrix(rep(t(solve(Sig_tmp) %*% rep(1, dat_test$K) %*% (1/(sum(solve(Sig_tmp))*dat_test$n_lakes)) %*% sum(z_test)), dat_test$n_lakes), ncol = dat_test$K, byrow = T)
...
par(mfrow = c(2,3))
for(i in 1:6){
  plot(chains$beta[,2,i], type = 'l', main = i)
}

par(mfrow = c(3,4))
for(i in 1:12){
  plot(chains$phi[,i,3], type = 'l', main = i)
}
...
covSigma = sum(((Sigma_species > slower) & (Sigma_species < supper))[lower.tri((Sigma_species > slower) & (Sigma_species < supper))])/length((((Sigma_species > slower) & (Sigma_species < supper))[upper.tri((Sigma_species > slower) & (Sigma_species < supper))]))
...
nlks = dim(lambdaTrue)[1]
nsmps = dim(chains$omega)[1]

lamHat = array(NA, dim = c(nlks, 6, nsmps))
...
for(i in 1:nsmps){
  ...
  lamHat[,,i] = computeLambda(NA, chains$beta[i,,], chains$phi[i,,], chains$Sigma_species[i,,],
                              chains$tau[i,], omega_test_hat, dat_test)
...
ggplot(preds, aes(x = exp(Predicted), y = exp(Observed), color = factor(year))) +
  geom_point() +
